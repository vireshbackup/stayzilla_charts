...
$(document).ready(function() {
    ...
    $('#topHostsToggle').click(function() {
        if ($("#topHostsToggleAction").text() == "Show") {
            ga('send', 'event', 'show', 'top hosts');
            $("#topHostsToggleAction").text("Hide");
            ...
            $("#topHostsToggleAction").text("Show");
...
function filterHighlyAvailable(filter) {
    if (availabilityClassificationFilterDimension){
        ...
        if (filter)
            availabilityClassificationFilterDimension.filterExact("high");
        else
        ...
        if (geoName == "") {
            ...
            neighbourhoodBoundaryFeatures.forEach(function(d){
                if (d.properties.neighbourhood == geoName) {
                    ...
                    if ((city != 'London') && (geoName != 'Topanga')){ //some bug for London, need to research
...
function loadListings(listingsFile) {
    d3.csv(listingsFile, function  (airbnb_data) {
        ...
        var roomTypeDimension = ndx.dimension(function (d) { return d.room_type; });
        ...
        roomTypeChart
            ...
            .group(roomTypeGroup)
            .colors(d3.scale.ordinal().domain(["Entire home/apt","Private room", "Shared room"])
        ...
        listingsPerHostChart
            ...
            .brushOn(false)
            .title(function(d) { 
                return "Number of listings where the host has " + (d.data.key - 1) + " other listing(s): " + formatThousands(d.data.value); 
            })
            .gap(0)
        ...
        listingsPerHostChart.xAxisLabel("listings per host");
        listingsPerHostChart.yAxisLabel("listings");
        ...
        availability365Chart
            ...
            .brushOn(false)
            .title(function(d) { 
                return "Number of listings with " + (d.data.key) + " day(s) available in the next 365 days: " + formatThousands(d.data.value); 
            })
        ...
        availability365Chart.xAxisLabel("number of days available in year");
        availability365Chart.yAxisLabel("listings");
        ...
        availabilityCategoriesPie
            ...
            .innerRadius(20)
            .colors(d3.scale.ordinal().domain(["high","low"])
                                        .range(["#b4b4b4", "#ccc"]))
            .renderTitle(true)
            .title(function(d) { 
                return "Number of listings with " + (d.data.key) + " availability: " + formatThousands(d.data.value); 
            }); 
        ...
        dc.renderAll("automaticRedrawGroup");
        addXAxisLabel(roomTypeChart, "listings");
...
function showHover(d){
    if (!hoverPinned){
        if (d3.select(d3.event.target).classed("unfiltered")){
            ...
            listingHover.select("#listingRoomType").text(d.room_type);
            ...
            if (d.number_of_reviews == 0)
                listingHover.select("#listingLastReview").text("N/A");
            else {
            ...
            listingHover.select("#listingAvailabilityDescription").text(d.availability_365 <= AVAILABILITY_DAYS_LOW ? "LOW" : "HIGH");
...
function saveViewState(){
    ...
    history.replaceState(viewState, "", "index.html?" + querystring);
...
function loadReviews() {
    ...
    d3.csv(reviewsFile, function  (airbnb_reviews) {
        ...
        reviewsChart
            ...
            .brushOn(false)
            .title(function(d) { 
                return "Number of reviews in " + (formatMonthYear(d.data.key)) + ": " + formatThousands(d.data.value); 
            })
            .x(d3.time.scale())
        ...
        reviewsChart.xAxisLabel("date");
        reviewsChart.yAxisLabel("reviews");
        reviewsChart.render(); 
